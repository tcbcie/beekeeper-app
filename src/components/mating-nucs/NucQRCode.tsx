...
interface NucQRCodeProps {
  tagCode: string
  nucNumber: string
}

export default function NucQRCode({ tagCode, nucNumber }: NucQRCodeProps) {
  ...
  const downloadQrCode = () => {
    const svg = document.getElementById('nuc-qr-code-svg')
    if (!svg) return
    ...
    img.onload = () => {
      ...
      downloadLink.download = `nuc-${nucNumber}-qr.png`
  ...
  const printQrCode = () => {
    ...
    const escapedNumber = escapeHtml(nucNumber)
    const escapedCode = escapeHtml(tagCode)

    printWindow.document.write(`
      <html>
        <head><title>Nuc ${escapedNumber} QR Code</title></head>
        <body style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;margin:0;font-family:sans-serif;">
          <h2>Nuc ${escapedNumber}</h2>
          ${svgData}
          <p style="color:#666;font-size:14px;margin-top:8px;">www.hivecraic.com</p>
          <p style="color:#333;font-size:16px;font-weight:bold;margin-top:2px;">${escapedCode}</p>
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <p className="text-sm text-text-tertiary">Nuc {nucNumber}</p>
      <QRCodeSVG
        id="nuc-qr-code-svg"
        value={qrUrl}
        size={200}
