...
export default function QueenRoleBadge({ role, className = '' }: { role?: string | null; className?: string }) {
  ...
  return (
    <span className={`px-2 py-0.5 text-xs font-medium rounded border border-amber-300 bg-amber-100 text-amber-800 dark:border-amber-700 dark:bg-amber-900/30 dark:text-amber-300 ${className}`}>
      {queenRoleLabel(role)}
    </span>
  )
}
