...
// Swarm Prediction Calculations
export type CellAge = 'eggs' | 'young_larvae' | 'old_larvae' | 'sealed'
export type CellType = 'swarm' | 'supercedure' | 'emergency'

export interface SwarmPrediction {
  predictedDate: Date
  daysRemaining: number
  swarmRisk: 'low' | 'medium' | 'high' | 'imminent'
  urgency: string
  actions: string[]
}

export function predictSwarmDate(
  ...
  if (!queenCellsPresent) {
    return {
      predictedDate: new Date(inspectionDate.getTime() + 14 * 24 * 60 * 60 * 1000),
      daysRemaining: 14,
      swarmRisk: 'low',
      urgency: 'No immediate swarm risk detected',
      actions: ['Continue weekly inspections during swarm season']
    }
  }

  // Development timeline (days until swarm)
  const cellTimelines = {
    eggs: 9,
    young_larvae: 6,
    old_larvae: 3,
    sealed: 0 // Swarm typically happens when first cell is sealed
  }

  const daysUntilSwarm = cellTimelines[cellAge]
  const predictedDate = new Date(inspectionDate)
  predictedDate.setDate(predictedDate.getDate() + daysUntilSwarm)

  // Calculate swarm risk
  let swarmRisk: 'low' | 'medium' | 'high' | 'imminent' = 'low'
  const actions: string[] = []

  if (cellType === 'swarm') {
    if (daysUntilSwarm === 0) {
      swarmRisk = 'imminent'
      actions.push('⚠️ URGENT: Swarm may leave today or tomorrow!')
      actions.push('Perform artificial swarm immediately or split colony')
      actions.push('Check for signs of swarm cells in other hives')
    } else if (daysUntilSwarm <= 3) {
      swarmRisk = 'high'
      actions.push('Inspect again in 2-3 days maximum')
      actions.push('Prepare to perform artificial swarm or split')
      actions.push('Add supers if nectar flow is on')
    } else if (daysUntilSwarm <= 6) {
      swarmRisk = 'medium'
      actions.push('Inspect again in 5-7 days')
      actions.push('Consider adding supers or expanding brood space')
      actions.push('Monitor queen cell development closely')
    } else {
      swarmRisk = 'low'
      actions.push('Inspect again in 7 days')
      actions.push('Ensure adequate space for brood and stores')
    }
  } else if (cellType === 'supercedure') {
    swarmRisk = 'low'
    actions.push('Colony is replacing queen - not a swarm risk')
    actions.push('Leave cells intact and monitor queen replacement')
    actions.push('Check for new queen laying in 3-4 weeks')
  } else {
    // Emergency cells
    swarmRisk = 'low'
    actions.push('Colony is queenless - emergency cells detected')
    actions.push('Leave cells intact unless introducing new queen')
    actions.push('Check for laying queen in 3-4 weeks')
  }

  // Factor in hive strength
  if (hiveStrength >= 4 && cellType === 'swarm') {
    if (swarmRisk === 'medium') swarmRisk = 'high'
    if (swarmRisk === 'low') swarmRisk = 'medium'
  }

  const urgency = swarmRisk === 'imminent'
    ? 'CRITICAL - Act within 24 hours'
    : swarmRisk === 'high'
    ? 'HIGH - Act within 2-3 days'
    : swarmRisk === 'medium'
    ? 'MODERATE - Act within one week'
    : 'LOW - Continue regular monitoring'

  return {
    predictedDate,
    daysRemaining: daysUntilSwarm,
    swarmRisk,
    urgency,
    actions
  }
