...
function LoginForm() {
  ...
  const handleGoogleSignIn = async (codeToValidate?: string) => {
    setLoading(true)
    setMessage('')

    try {
      // Check if user needs to provide registration code
      const { data: { session } } = await supabase.auth.getSession()
      const isExistingUser = session !== null

      // For new users (not signed in), require registration code
      if (!isExistingUser) {
        const code = codeToValidate || googleCodeInput.trim()
  ...
  const handleGoogleCodeSubmit = (e: React.FormEvent) => {
    ...
    if (googleCodeInput.trim()) {
      handleGoogleSignIn(googleCodeInput.trim())
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-yellow-50 to-amber-100">
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow-lg">
        ...
        <div className="flex items-center justify-center gap-3 text-xs text-gray-500">
          <span className="px-2 py-1 bg-amber-50 text-amber-700 rounded font-medium">v1.0.25</span>
          <span>•</span>
