...
## 2. Scope & Simplicity
* **In Scope:** Add a new planning tab on `/dashboard/batches`, show queen timeline milestones with weekdays, and derive the linked drone preparation window from the selected graft date.
...
The planner will use one primary graft date input and display the derived milestones in a readable timeline-style layout. Initial assumptions will be explicit in the UI, with weekday labels surfaced alongside each date so the user can compare practical working days at a glance.

The first pass will calculate:
- queen emergence from the selected graft date, aligned with the existing Queen Rearing timeline already used on the batches form
