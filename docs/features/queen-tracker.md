...
- **Actions:** `Mated`, `Hybridised`, `Overwintered`, and `Failed` share one compact action column between `Queen` and `Status`, with a shallow label layer and a denser two-by-two control grid beneath it, followed by the inline date-capture editor when the user records a date-bearing outcome
