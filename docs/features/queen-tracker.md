...
- **Queen:** A unique queen identity that leads with the tagged queen number (`Queen 38`) when the queen is tagged, and falls back to `Batch · Cell N` when it is not — because a bare cell number only counts within its own batch and repeats across batches. A compact marked indicator sits beside the identity, followed by a `Breeder N · subspecies` lineage line (omitted when no breeder is known), then an explicit `Age ...` summary line, plus a selected-row treatment so the active row stays obvious
