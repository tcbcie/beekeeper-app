...
* **Files to Modify:** * `src/hooks/useQueenTracker.ts`
  ...
  * `docs/features/queen-ledger-hardening-remediation-plan.md`
  * `tasks/todo-codex.md`
...
## Review
* **Scope Covered:** Queen Ledger hardening remediation pass.
* **Summary of Changes:** The ledger now queries only the rows the current user should see, avoids duplicate weight lookups, skips malformed rows instead of fabricating placeholder cell identities, and keeps the hybridisation date input aligned with persisted state.
* **Notes for User:** Please check the Queen Ledger in your normal build flow, especially owner visibility versus member visibility, records with missing graft metadata, and hybridisation date editing including clearing and failed-save recovery.
