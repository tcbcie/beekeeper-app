...
* **Simplicity Check:** This is a surgical TypeScript fix in the shared mating helper. It does not change the mating workflow, data model, or database writes; it only makes the confirmed-date branch type-safe so the build compiles cleanly.
...
## Review
* **Scope Covered:** Queen Ledger mating helper build fix.
* **Summary of Changes:** The shared mating helper now narrows the confirmed-date branch before validation, which removes the TypeScript error without changing any ledger or distribution behaviour.
