...
## Review
* **Scope Covered:** Queen Ledger distribution-column refinement.
* **Summary of Changes:** The summary row now reads `Details`, `Queen`, `Actions`, `Status`, and `Distribution`, and the distribution cell focuses on the recipient plus a compact `Group Member`, `App User`, or `Public Recipient` cue.
* **Notes for User:** Please test the Queen Ledger in your normal build flow, especially recipient-type labelling for group distributions, app-user distributions, and public distributions.
