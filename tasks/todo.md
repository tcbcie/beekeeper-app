...
* **Summary of Changes:** Added guarded `auth.users` extraction in admin export and generated **ID-only** idempotent auth seed statements (not full-row inserts) wrapped in a runtime check for `auth.users` availability; added summary diagnostics for auth export status. Also hardened metadata/auth RPC query text to avoid `execute_safe_query` substring guard false-positives (for example `DROP` in `dropdown_*` and `CREATE` in `created_at`) that caused export route 500 errors, and excluded dropped physical columns from generated table DDL using `a.atttypid <> 0` after a local restore syntax failure (`........pg.dropped...`).
