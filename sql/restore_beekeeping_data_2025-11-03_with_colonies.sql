-- ============================================================================
-- RESTORE BEEKEEPING DATA FROM 2025-11-03 BACKUP (WITH COLONY MIGRATION)
-- ============================================================================
-- This script restores all beekeeping data from the CSV export
-- AND creates colony records for the restored hives
-- Order matters: apiaries → queens → hives → colonies → inspections → varroa checks/treatments
-- ============================================================================
...
-- 3. RESTORE HIVES (11 records) - WITHOUT colony_id initially
...
-- ============================================================================
-- 4. CREATE COLONIES FOR RESTORED HIVES
-- ============================================================================
-- This section creates colony records for each restored hive
-- and links them together properly
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Creating colonies for restored hives...';
  RAISE NOTICE '========================================';

  -- For each restored active hive without a colony
  FOR hive_record IN
    SELECT
      h.id as hive_id,
      h.user_id,
      ...
      h.hive_type
    FROM hives h
    WHERE h.user_id = '08e38bd9-30b0-4183-92c2-fc3b7600a46a'
      AND h.status = 'active'
  ...
  LOOP
    ...
    BEGIN
      ...
      ELSIF hive_record.hive_type = 'Nuc' THEN
        v_origin_type := 'nuc';
      ELSE
        v_origin_type := 'other';  -- Default for Production hives
      END IF;
      ...
      INSERT INTO colonies (
        ...
        'COL-' || hive_record.hive_number,  -- Use hive number as basis
        hive_record.user_id,
        v_origin_type,
        COALESCE(hive_record.colony_established_date, hive_record.created_at::date),
        'active',
        'Colony restored from backup (2025-11-03) for hive ' || hive_record.hive_number,
        hive_record.created_at
      )
      ON CONFLICT (colony_number) DO NOTHING  -- Skip if already exists
      RETURNING id INTO new_colony_id;

      -- If insert succeeded, update the hive
      IF FOUND THEN
        UPDATE hives
  ...
  RAISE NOTICE '========================================';
  RAISE NOTICE '✅ Created % colonies', colony_count;
  RAISE NOTICE '========================================';
END $$;

-- ============================================================================
-- 5. RESTORE INSPECTIONS (5 records) - WITHOUT colony_id initially
-- ============================================================================
...
-- 6. RESTORE VARROA CHECKS (1 record)
...
-- 7. RESTORE VARROA TREATMENTS (1 record)
...
-- ============================================================================
-- 8. BACKFILL colony_id ON ALL RESTORED RECORDS
-- ============================================================================
-- This links inspections, varroa checks, and treatments to their colonies
...
BEGIN
  ...
  WHERE i.hive_id = h.id
    ...
    AND i.user_id = '08e38bd9-30b0-4183-92c2-fc3b7600a46a';
  ...
  WHERE vt.hive_id = h.id
    ...
    AND vt.user_id = '08e38bd9-30b0-4183-92c2-fc3b7600a46a';
  ...
  WHERE vc.hive_id = h.id
    ...
    AND vc.user_id = '08e38bd9-30b0-4183-92c2-fc3b7600a46a';
...
-- ============================================================================
-- 9. CREATE INITIAL COLONY MOVEMENT RECORDS
-- ============================================================================
...
BEGIN
  ...
  -- Create a "new_colony" movement for each restored colony
  ...
  SELECT
    ...
    c.origin_date,
    'new_colony',
    'Initial placement (restored from 2025-11-03 backup)'
  FROM colonies c
  JOIN hives h ON h.colony_id = c.id
  WHERE c.user_id = '08e38bd9-30b0-4183-92c2-fc3b7600a46a'
    AND NOT EXISTS (
...
UNION ALL
SELECT 'Colonies', COUNT(*) FROM public.colonies WHERE user_id = '08e38bd9-30b0-4183-92c2-fc3b7600a46a'
UNION ALL
SELECT 'Colony Movements', COUNT(*) FROM public.colony_movements WHERE user_id = '08e38bd9-30b0-4183-92c2-fc3b7600a46a'
UNION ALL
...
-- Show colonies with their hives
SELECT
  c.colony_number,
  c.origin_type,
  c.origin_date,
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_colonies FROM public.colonies WHERE user_id = '08e38bd9-30b0-4183-92c2-fc3b7600a46a';
  ...
  RAISE NOTICE '✅ Colonies created: %', v_colonies;
  RAISE NOTICE '✅ Inspections restored: %', v_inspections;
  RAISE NOTICE '========================================';
  RAISE NOTICE 'All beekeeping data has been restored!';
  RAISE NOTICE 'Colony tracking is now fully integrated.';
  RAISE NOTICE '========================================';
  RAISE NOTICE '';
END $$;
