...
COMMENT ON COLUMN public.user_profiles.association_id IS 'Reference to local beekeeping association (if member of one)';
COMMENT ON COLUMN public.user_profiles.member_fibka IS 'Member of Federation of Irish Beekeepers Associations (FIBKA)';
COMMENT ON COLUMN public.user_profiles.member_iba IS 'Member of Irish Beekeepers Association (IBA)';
COMMENT ON COLUMN public.user_profiles.member_nihbs IS 'Member of Native Irish Honey Bee Society (NIHBS)';
